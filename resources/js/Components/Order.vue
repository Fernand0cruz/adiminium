    ...
    <div class="border p-4 rounded-lg">
        <div v-for="product in order[0].products" :key="product.id"
            ...
            <div class="flex-grow px-4">
                ...
                <div class="text-sm text-gray-600">
                   Preço da unidade:
                    {{ formatCurrency((product.pivot.price))}} - {{ product.pivot.discount }}% de desconto =
                    <span class="font-bold">{{formatCurrency((product.pivot.price * product.pivot.quantity) * (1 - product.pivot.discount / 100)) }}</span>
                </div>
            </div>
        ...
        <div class="flex justify-between items-center mt-4">
            <div class="text-lg font-semibold">
                Preço total do Pedido: {{ formatCurrency(order[0].total_price) }}
            </div>
            <div>
                <Link href="route('#')"
